 ...
 *
 */
router.delete('/access-card/:id/notes/:noteId', (req: Request, res: Response) => {
  AccessCard.update({ _id: req.params.id }, { $pull: { notes: { _id: req.params.noteId } } }, function(err: Error, data: {}) {
    if (err) {
      res.json(err);
    } else {
