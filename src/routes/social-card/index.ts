 ...
 *
 */
router.delete('/social-card/:id/notes/:noteId', (req: Request, res: Response) => {
  SocialCard.update({ _id: req.params.id }, { $pull: { notes: { _id: req.params.noteId } } }, function(err: Error, data: {}) {
    if (err) {
      res.json(err);
    } else {
